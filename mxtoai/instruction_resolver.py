...
class ProcessingInstructionsResolver:
        ...
        for instructions in instructions_list:
            ...
            for name in all_names:
                if name in self.handle_map and not overwrite:
                    raise ValueError(f"Handle or alias '{name}' already registered. Use `overwrite=True` to replace.")
    ...
    def list_available_handles(self) -> List[str]:
        ...
        return list(set(instr.handle for instr in self.handle_map.values()))
