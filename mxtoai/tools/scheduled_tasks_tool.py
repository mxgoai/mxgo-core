...
from mxtoai.db import init_sync_db_connection
...
class ScheduledTaskInput(BaseModel):
    ...
    def validate_email_request(self, v):
        """Validate that email_request contains necessary data"""
        if not isinstance(v, dict):
            msg = "email_request must be a dictionary"
            raise ValueError(msg)
        if not v.get("from"):
            msg = "email_request must contain 'from' field"
            raise ValueError(msg)
        return v
...
class ScheduledTasksTool(Tool):
    ...
    inputs = {
        ...
        "task_description": {"type": "string", "description": "Human-readable description of what the task does"},
        "next_run_time": {
            "type": "string",
    ...
    def forward(
        ...
        try:
            ...
            error_msg = f"Failed to store scheduled task: {e!s}"
            logger.error(error_msg, exc_info=True)
            return {"status": "error", "message": error_msg, "task_description": task_description}
